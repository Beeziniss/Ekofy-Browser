...
export function SubscriptionPlanCard({ 
  ...
  const getCorrectPrice = () => {
    ...
    if (selectedInterval === 'monthly') {
      ...
      return prices.find(p => p.interval === 'YEAR' || (p.interval === 'MONTH' && p.intervalCount === 12)) || prices[0];
    }
  };
  ...
  const getCardStyling = () => {
    if (isPro) {
      return {
        ...
        buttonClass: 'bg-gradient-to-r from-purple-500 to-blue-500 hover:from-purple-600 hover:to-blue-600 text-white shadow-lg border-none',
  ...
  return (
    <Card className={`relative overflow-hidden transition-all duration-300 hover:shadow-2xl hover:scale-105 ${styling.cardClass}`}>
      {/* Gradient border for premium/pro */}
      {(isPremium || isPro) && (
        <div className={`absolute inset-0 ${styling.borderGradient} p-[2px] rounded-lg`}>
          <div className={`${styling.bgClass} rounded-lg h-full w-full`} />
        </div>
      )}

      <CardHeader className="text-center space-y-4 relative z-10 pb-3">
        {/* Title and Icon */}
        <div className="flex items-center justify-center space-x-3">
          <div>
            <CardTitle className={`text-2xl font-bold ${styling.textColor}`}>
              {plan.stripeProductName}
            </CardTitle>
            {(isPremium || isPro) && (
              <Badge className={`mt-1 ${styling.badgeClass}`}>
                {styling.badgeText}
              </Badge>
            )}
          </div>
        ...
        {subscription.description && (
          <p className={`text-sm ${styling.subTextColor}`}>
            {subscription.description}
          </p>
        )}
      </CardHeader>
      ...
      <CardContent className="space-y-3 relative z-10">
        ...
        <div className="flex justify-center">
          <div className={`flex rounded-[999px] p-1 ${styling.toggleBg}`}>
            <button
              ...
              className={`px-4 py-2 rounded-[999px] text-sm font-medium transition-all ${
                selectedInterval === 'monthly'
                  ? styling.toggleActiveClass
                  : styling.toggleInactiveClass
              }`}
            >
              Monthly
            </button>
        ...
        <div className="text-center">
          ...
          {shouldApplyCoupon && (
            <div className="flex items-center justify-center space-x-2 mt-2">
              ...
              <Badge className="bg-green-500 text-white text-xs border-none">
                Save {couponDiscount}%
              </Badge>
            </div>
          )}
          ...
          <ul className="space-y-3">
            {features.slice(0, 4).map((feature, index) => (
              <li key={index} className="flex items-start space-x-3">
                <div className={`p-1 rounded-full ${styling.featureIconBg}`}>
                  <Check className={`w-3 h-3 ${styling.featureIconColor}`} />
                </div>
                <span className={`text-sm ${styling.featureTextColor}`}>
                  {feature}
                </span>
              </li>
            ))}
        ...
        {/* Action Button */}
        <Button 
          className={`w-full h-12 text-base font-semibold transition-all duration-200 ${styling.buttonClass}`}
