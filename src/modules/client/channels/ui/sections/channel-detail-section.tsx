...
import { trackPublicInfiniteWithCategoryOptions } from "@/gql/options/client-options";
...
const ChannelDetailSection = ({ channelId }: ChannelDetailSectionProps) => {
  return (
    <Suspense fallback={<ChannelDetailSectionSkeleton />}>
      <ChannelDetailSectionSuspense channelId={channelId} />
    </Suspense>
  );
};

const ChannelDetailSectionSkeleton = () => {
  return (
...
const ChannelDetailSectionSuspense = ({ channelId }: ChannelDetailSectionProps) => {
  ...
  const { data, isPending, hasNextPage, isFetchingNextPage, fetchNextPage } = useInfiniteQuery(
    trackPublicInfiniteWithCategoryOptions(12, channelId),
  );
...
export default ChannelDetailSection;
